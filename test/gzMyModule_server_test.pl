...
my $config = new Config::Simple($config_file)->get_block('gzMyModule');
my $ws_url = $config->{"workspace-url"};
my $ws_name = undef;
my $ws_client = new Bio::KBase::workspace::Client($ws_url,token => $token);
my $auth_token = Bio::KBase::AuthToken->new(token => $token, ignore_authrc => 1);
my $ctx = LocalCallContext->new($token, $auth_token->user_id);
$gzMyModule::gzMyModuleServer::CallContext = $ctx;
my $impl = new gzMyModule::gzMyModuleImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_gzMyModule_' . $suffix;
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'gzMyModule', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
