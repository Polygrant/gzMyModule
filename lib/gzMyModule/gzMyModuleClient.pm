package gzMyModule::gzMyModuleClient;
...
sub new
{
    ...
    my $self = {
	client => gzMyModule::gzMyModuleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a gzMyModule.FilterContigsParams
$return is a gzMyModule.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a gzMyModule.workspace_name
	contigset_id has a value which is a gzMyModule.contigset_id
	min_length has a value which is an int
workspace_name is a string
contigset_id is a string
FilterContigsResults is a reference to a hash where the following keys are defined:
	new_contigset_ref has a value which is a gzMyModule.ws_contigset_id
...
# Authentication: required
	...
	method => "gzMyModule.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for gzMyModule::gzMyModuleClient\n";
    }
    if ($sMajor == 0) {
        warn "gzMyModule::gzMyModuleClient version is $svr_version. API subject to change.\n";
    }
}
...
package gzMyModule::gzMyModuleClient::RpcClient;
