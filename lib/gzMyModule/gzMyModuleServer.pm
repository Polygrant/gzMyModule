package gzMyModule::gzMyModuleServer;
...
our %async_run_methods = (
        'filter_contigs_async' => 'gzMyModule.filter_contigs',
);

our %async_check_methods = (
        'filter_contigs_check' => 'gzMyModule.filter_contigs',
);
...
sub call_method {
    ...
    my $ctx = gzMyModule::gzMyModuleServerContext->new($self->{loggers}->{userlog},
...
{
    # Service gzMyModule requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for gzMyModule but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = gzMyModule::gzMyModuleServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service gzMyModule.");
    }
...
package gzMyModule::gzMyModuleServerContext;

use strict;

=head1 NAME

gzMyModule::gzMyModuleServerContext
...
package gzMyModule::gzMyModuleServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = gzMyModule::gzMyModuleServer->new(
